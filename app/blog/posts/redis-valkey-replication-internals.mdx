...
Now we are in a state of inconsistency! Master says that `myhash` has a field called `f1` which is expired however the replica still contains this key, with what TTL exactly? Since the replicated command only has `KEEPTTL` (and the original TTL already expired on the replica), this key won't have any TTL on the replica and will live infinitely (by the time you're reading this blog this has already been merged and fixed we'll talk about it soon ;)
...
<Mermaid chart={`sequenceDiagram
    ...
    C->>M: HSETEX EX 3 myhash fields f1 v1
    M->>M: Set f1 with TTL=3s
    M->>R: Replicate: HSETEX EX 3 myhash fields f1 v1
    R->>R: Set f1 with TTL=3s
    
    Note over M,R: Network delay occurs...
    Note over R: TTL expires on replica (4s passed)
    R->>R: f1 expired and deleted
    
    C->>M: HSETEX myhash KEEPTTL fields f1 v1
    M->>M: Set f1, keep existing TTL
    M->>R: Replicate: HSETEX myhash KEEPTTL fields f1 v1
    R->>R: Set f1 with KEEPTTL but no TTL exists!
    
    Note over M: f1 has TTL (will expire)
    Note over R: f1 has NO TTL (lives forever!)
`} caption="The KEEPTTL bug: network delays can cause TTL inconsistencies between master and replica" />
...
<Mermaid chart={`flowchart TB
    ...
    M --> RB
    RB -->|async propagation| R1
    RB -->|async propagation| R2
`} caption="Primary-Replica architecture: writes go to master, reads can be served by replicas" />
...
Valkey also is very smart about replication the master maintains something called a "Repl Backlog" which is the amount of RESP commands that are stored in the buffer this is something that is maintained by the primary which means if the client requests any command that was executed before the available backlog Valkey falls back to a full resync where it requests a full copy of the data through RDB (one of the two persistance options the other being AOF).
...
2. Active Seek -- Valkey runs a cron job in the background which runs 10 times every second sampling a small set of keys with expiration and deletes the ones that have already expired.
...
<Mermaid chart={`flowchart TB
    subgraph LinkedList[Replication Buffer - Linked List]
        direction LR
        B1["Block 0<br/>refcount: 1<br/>offset: 0"] --> B2["Block 1<br/>refcount: 0<br/>offset: 1024"] --> B3["Block 2<br/>refcount: 2<br/>offset: 2048"]
    end
    
    BL[Repl Backlog] -.->|references| B1
    RA[Replica A] -.->|references| B3
    RB[Replica B] -.->|references| B3
`} caption="replBufBlock linked list: each block tracks its reference count, allowing safe trimming from the head" />
...
Observe in the above struct we have `replid` and `repl_transfer_last_fsync_off`. The `replid` is used by the replica with the PSYNC command to the primary, while `repl_transfer_last_fsync_off` tracks periodic disk flushes during RDB loading. In the function `replicaReceiveRDBFromPrimaryToDisk` we have the following:
...
<Mermaid chart={`flowchart TB
    ...
    B --> C[connSocketEventHandler]
    C --> D[readQueryFromClient]
    
    subgraph CMD[Command Processing]
    ...
    subgraph PROP[Propagation]
        G --> H[afterCommand]
        H --> I[propagateNow]
        I --> J[replicationFeedReplicas]
    end
`} caption="Command execution flow: from event loop to replication propagation" />
...
There are these 3 statements each with some specific job, if you recall about AOF the first call does exactly what it says its for AOF persistence and writes the commend provided in argv into the AOF file. The second is what we're most interested in.

<Mermaid chart={`flowchart TB
    P[propagateNow]
    
    P --> C1{propagate_to_aof?}
    P --> C2{propagate_to_repl?}
    P --> C3{propagate_to_slot_migration?}
    
    C1 -->|yes| AOF[feedAppendOnlyFile]
    C2 -->|yes| REPL[replicationFeedReplicas]
    C3 -->|yes| SLOT[clusterFeedSlotExportJobs]
    
    AOF --> D1[(AOF File)]
    
    REPL --> FB[feedReplicationBuffer]
    FB --> RB[replBufBlock List]
    RB --> R1[Replica 1]
    RB --> R2[Replica 2]
    
    SLOT --> MIG[Slot Migration Jobs]
`} caption="propagateNow: commands flow to AOF, replicas, and cluster slot migrations" />

The comment above the function says the following:
